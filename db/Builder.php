...
class Builder extends Command
{
	 ...
	 */
	public static function getColumnInfo($type)
	{
		$col = ''; //列信息
		$typeMap = self::getTypeMap();
		if(is_array($type) && isset($typeMap[$type[0]])){//必须为数组，并且类型必须存在
			$comment = isset($type[1])?' COMMENT \''.$type[1].'\'':''; //备注
			switch ($type[0]){
				case 'pk': case 'bigpk':
					$col = $typeMap[$type[0]].$comment;
					break;
				case 'string':
					$col = $typeMap[$type[0]];
				...
				default:
					...
					$col = $typeMap[$type[0]].(isset($type[2]) && $type[2]==1?' NOT NULL':'').$default.$comment;
			}
		}else{
			if(is_array($type)){
	 ...
	 */
	public static function getCreateTableSql($table, $columns, $charset = 'utf8',$engine = 'InnoDB')
	{
		$cols = [];
		foreach ($columns as $name => $type) {
			if (is_string($name)) {
				$cols[] = "  `" . $name . '` ' . self::getColumnInfo($type);
			} else {
				$cols[] = '  '.$type;
			}
		}
		...
		return $isdrop."CREATE TABLE `" . $tname . "` (".PHP_EOL . implode(",".PHP_EOL, $cols) .PHP_EOL.") ENGINE=".$engine." DEFAULT CHARSET=".$charset.$tcomment;
	}
	/**
	 * 创建新表
	 * 例如,
	...
	{
		$sql = self::getCreateTableSql($table,$columns,$charset,$engine);
	...
	{
		self::setBuildSql('ALTER TABLE `'.$table.'` ADD `'.$column.'` '.self::getColumnInfo($type));
		return $this;
	}
	...
	{
		self::setBuildSql('ALTER TABLE `'.$table.'` CHANGE `'.$oldName. '` `'.$newName.'` '.self::getColumnInfo($type));
		return $this;
	}
	/**
	 * 构建一个SQL语句添加到现有表主键
